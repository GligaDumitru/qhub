...
export async function toggleSaveQuestion(params: CollectionBaseParams): Promise<ActionResponse<{ saved: boolean }>> {
  ...
  try {
    ...
    if (!question) {
      return handleError(new NotFoundError("Question not found")) as ErrorResponse;
    }

    const existingCollection = await Collection.findOne({ author: userId, question: questionId });
...
export async function hasSavedQuestion(params: CollectionBaseParams): Promise<ActionResponse<{ saved: boolean }>> {
  ...
  try {
    const existingCollection = await Collection.findOne({ author: userId, question: questionId });
    return { success: true, data: { saved: !!existingCollection } };
  } catch (error) {
    return handleError(error) as ErrorResponse;
  }
}
...
export async function getSavedQuestions(
  ...
  try {
    ...
    const [totalCount = { count: 0 }] = await Collection.aggregate([...pipeline, { $count: "count" }]);
